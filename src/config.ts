...
export async function loadStartupConfig(): Promise<JinConfig> {
  return withConfigLock(async () => {
    ...
    if (materialized.changed) {
      await writeConfigFile(materialized.value);
    }
...
): Promise<{ config: JinConfig; result: T }> {
  return withConfigLock(async () => {
...
async function writeConfigFile(config: unknown): Promise<void> {
  ...
  const tmpPath = `${cfgPath}.${process.pid}.${Date.now()}.tmp`;
...
async function acquireConfigLock(timeoutMs = CONFIG_WRITE_TIMEOUT_MS): Promise<() => void> {
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(
          `Timed out waiting for config lock at ${lockPath}`,
        );
      }
