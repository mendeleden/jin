  ...
  try {
    const handle = await runPipeline({
      ...
      onConfigReload: async (source) => {
        const nextConfig = await reloadRuntimeConfig(currentConfig, currentSinks, {
          source,
          log,
        });
        if (!nextConfig) {
        ...
        currentConfig = nextConfig.config;
        currentSinks = nextConfig.sinks;
      },
      workerIngest: {
...
async function reloadRuntimeConfig(
  ...
  try {
    ...
    options.log(`WARNING: Config reload failed; keeping prior runtime view. ${formatError(error)}`);
    return null;
  }
}
