...
export async function sinkAddCommand(
  ...
  const { result } = await updateConfig(async (config) =>
    ensureSinkConfigured(config, { ...opts, type }),
  );
...
async function setSinkEnabled(sinkId: string, enabled: boolean): Promise<void> {
  ...
  const { config, result } = await updateConfig((nextConfig) => {
    const sinkIndex = findSinkIndexById(nextConfig, sinkId);
    if (sinkIndex === -1) {
      fail(`sink "${sinkId}" not found`);
    }

    const current = nextConfig.sinks[sinkIndex];
    const alreadyEnabled = current.enabled !== false;
    if (alreadyEnabled === enabled) {
      return { changed: false };
    }

    nextConfig.sinks[sinkIndex] = {
      ...current,
      enabled,
    };
