...
- title: `Bump to 0.8.5 and Local Service Rollout`
- status: `blocked`
...
- unblocks: `local 0.8.5 service-mode validation on a clean baseline`
- last transition: `2026-04-08`
- next Codex action: `wait for W3-PERF-04 runtime stability and W3-SINK-04 sink proof before resuming service-mode smoke validation`
- latest review: `none`
...
- this lane runs from an isolated worktree on the approved baseline so it does
  ...
  workspace
- the lane should bump the product version to `0.8.5`, rebuild/install the
  binary, and bring the local launchd service up with bounded verification
- the version/build/install slice already happened locally; the remaining service
  validation is blocked on runtime RSS stability and honest sink proof
