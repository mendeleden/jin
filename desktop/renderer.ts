...
const USAGE_SHARE_BUCKETS = [
  1, 2, 3, 4, 5, 6, 8, 10, 12, 15, 18, 20, 25, 30, 35, 40, 45, 50, 55, 60,
            ...
            return `
              <div class="usage-row">
                ...
                <div class="usage-bar-track" title="${escapeHtml(
                  ...
                  ${sortedEntries
                    .map((entry) => {
                      const widthClass = usageShareClass(
                        (entry.tokens / maxDailyTokens) * 100,
                      );
                      return `
                        <span
                          class="usage-bar-segment ${usageColorClass(
...
function usageShareClass(width: number): string {
  if (!Number.isFinite(width) || width <= 0) {
    return "usage-share-0";
  }

  const share =
    USAGE_SHARE_BUCKETS.find((bucket) => width <= bucket) ??
    USAGE_SHARE_BUCKETS[USAGE_SHARE_BUCKETS.length - 1];
  return `usage-share-${share}`;
}
